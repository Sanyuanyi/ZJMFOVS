...
def compute_burst_kb(rate_kbps: int) -> int:
    ...
    计算 burst：
    - 不小于速率的 10%。
    - 不小于 2000kb（避免过小影响长肥管道）。
    - 不超过 1 秒带宽（rate_kbps）。
    """
    if rate_kbps <= 0:
        return 0
    burst = max(int(rate_kbps * 0.1), 2000)
    return min(burst, rate_kbps)
...
def cmd_list() -> None:
    ...
    state = load_state()
    if not state:
        print("No state found (daemon not running or no interfaces).")
        return
