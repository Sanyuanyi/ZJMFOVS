...
6. CLI 查询：`python3 ovs_qos_optimized_final_py36.py list` 查看各接口当前阶段、限速、剩余惩罚时间和上一采样 Mbps。
...
`sample_interval_sec` | 300 | 采样间隔（秒）。
...
2. 放置程序与配置目录  
   ...
   chmod 750 /opt/ovs-qos/ovs_qos_optimized_final_py36.py
   # 如需自定义配置
...
- 查看限速状态：`python3 /opt/ovs-qos/oovs_qos.py list`
...
- 突发控制：`burst = min(max(10%速率, 2000kb), 1 秒带宽)`，兼顾 TCP 启动与过度突发。
